...
from utils import move_to, EarlyStopping
...
class Trainer:
    ...
    def reset_state(self, params: dict = {}):
        """
        Resets state with the with the attributes of the Trainer.

        params: dict: params to override or additional params to add to the state.
        """
        self.state = {}
    ...
    def train_epoch(self):
        ...
        for batch in self.train_dataloader:
            ...
            if self.state["train_it"] % self.state["epoch_len"] == 0:
                ...
                if "early_stopping" in self.state and self.state["early_stopping"].should_stop(self):
                    if self.state.get("min_train_iterations", 0) <= self.state["train_it"]:
    ...
    def train(self, min_epochs=None, max_epochs=None, epoch_len=None, lr=None, early_stopping: Optional[EarlyStopping] = None, accumulation_steps=1):
        ...
        if max_epochs:
            self.state["max_train_iterations"] = current_it + max_epochs * self.state["epoch_len"]
        if min_epochs:
            self.state["min_train_iterations"] = current_it + min_epochs * self.state["epoch_len"]
        ...
        if early_stopping:
            self.state["early_stopping"] = early_stopping
    ...
    def load(self, identifier=None, load_optimiser=True, exclude_keys=[], dir="saved_models"):
        """
        param exclude_keys: list of keys in the model and optimiser dictionaries to exclude.
                            Useful in cases where the model loaded to has some different layers.
        """

        if identifier is None:
            identifier = self.get_id()

        path = Path(__file__).parent / dir / f"{identifier}.pt"
